...
| `nowcast_rate_impact.json` | Historical `simulate_nowcast_rate_impact.py`, retained in [repository history](https://github.com/PolicyEngine/spm-threshold-paper/blob/8d8a8b4960eb22e207e5e4e735277754ddf12cad/scripts/simulate_nowcast_rate_impact.py) | PolicyEngine US microsimulation (policyengine-us, editable install of spm-calculator from the PR #32 branch), default Populace dataset as of 2026-07-18 | Frozen poverty-rate sensitivity: CPI-U-aged vs nowcast 2025 thresholds. The committed result predates the linked script revision and was written to a scratch path. Its recorded environment lacks an immutable dataset identity, so exact replay is not established. The obsolete executable was removed from current scripts; the result, original source history and timestamp commitments remain unchanged. |
